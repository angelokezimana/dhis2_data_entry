                ...
                <div class="overflow-hidden">
                    <table class="min-w-full text-left text-sm font-light">
                        <thead class="border-b font-medium dark:border-neutral-500">
                            <tr>
                                <th scope="col" class="px-6 py-3">Date occurred</th>
                                <th scope="col" class="px-6 py-3">Dataset</th>
                                <th scope="col" class="px-6 py-3">Data element</th>
                                <th scope="col" class="px-6 py-3">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($hiv_testings as $hiv_testing)
                                <tr wire:key="{{ $hiv_testing->id }}" class="border-b dark:border-neutral-500">
                                    <td class="whitespace-nowrap px-6 py-4">
                                        {{ \Carbon\Carbon::parse($hiv_testing->date_occurred)->format('d/m/Y h:i a') }}
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">{{ $hiv_testing->dataset }}</td>
                                    <td class="whitespace-nowrap px-6 py-4">{{ $hiv_testing->data_element }}</td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <button type="button"
                                            wire:click="$dispatch('set-details-patient', { hiv_testing_id: '{{ $hiv_testing->id }}' })"
                                        ...
                                        <button type="button" wire:click="delete('{{ $hiv_testing->id }}')"
